...
describe YayImdbs do
  ...
  it 'should find movie imdb id with name and year' do
    ...
    YayImdbs.should_receive(:get_search_page).with(movie_name).and_return(open(File.join(File.dirname(__FILE__), 'starkey_hutch_search.html')) { |f| Hpricot(f) })
  ...
  it 'should find the imdb id when search redirects directly to the movie page' do
    movie_name = 'Avatar'
    YayImdbs.should_receive(:get_search_page).with(movie_name).and_return(open(File.join(File.dirname(__FILE__), 'Avatar.2009.html')) { |f| Hpricot(f) })
  ...
  it 'should search imdb and return name, year and id even for exact tv show search result' do
    movie_name = 'Lost'
    YayImdbs.should_receive(:get_search_page).with(movie_name).and_return(open(File.join(File.dirname(__FILE__), 'Lost.2004.html')) { |f| Hpricot(f) })
  ...
  it 'should detect tv show type' do
    pending 'gets encoding error in ruby 1.9'
    imdb_id = '0411008'
    YayImdbs.should_receive(:get_movie_page).with(imdb_id).and_return(open(File.join(File.dirname(__FILE__), 'Lost.2004.html')) { |f| Hpricot(f) })
    YayImdbs.should_receive(:get_episodes_page).with(imdb_id).and_return(open(File.join(File.dirname(__FILE__), 'Lost.2004.Episodes.html')) { |f| Hpricot(f) })
  ...
  it 'should detect movie type' do
    imdb_id = '0499549'
    YayImdbs.should_receive(:get_movie_page).with(imdb_id).and_return(open(File.join(File.dirname(__FILE__), 'Avatar.2009.html')) { |f| Hpricot(f) })
